...
class Collision {
    ...
    static func checkDirection(subject r1: float4, target r2: float4, velocity: float2) -> Direction {
        ...
        let tr = check(point: [r1[2], r1[1]], rect: r2)
        ...
        if tl && !tr && !bl && !br {
            let a = getAngle([r2[2], r2[3]], [previousRect[0], previousRect[1]])
            if a > -PI/4 && a < 3*PI/4 { return Direction.right } else { return Direction.down }
        }
        if !tl && tr && !bl && !br {
            let a = getAngle([r2[0], r2[3]], [previousRect[2], previousRect[1]])
            if a > -3*PI/4 && a < PI/4 { return Direction.down } else { return Direction.left }
        }
        if !tl && !tr && bl && !br {
            let a = getAngle([r2[2], r2[1]], [previousRect[0], previousRect[3]])
            if a > -3*PI/4 && a < PI/4 { return Direction.right } else { return Direction.up }
        }
        if !tl && !tr && !bl && br {
            let a = getAngle([r2[0], r2[1]], [previousRect[2], previousRect[3]])
