...
class Collision {
    ...
    static func checkDirection(subject r1: float4, target r2: float4, velocity: float2) -> Direction {
        ...
        // corner collisions
        if tl && !tr && !bl && !br {
            let a1 = getAngle([r1[0], r1[1]], [r2[2], r2[3]])
            let a2 = getAngle([r1[2], r1[3]], [r2[2], r2[3]])
            if a1 > a2 { return Direction.right } else { return Direction.down}
        }
        if !tl && tr && !bl && !br {
            let a1 = getAngle([r1[2], r1[1]], [r2[0], r2[3]])
            let a2 = getAngle([r1[0], r1[3]], [r2[0], r2[3]])
            if a1 > a2 { return Direction.down } else { return Direction.left }
        }
        if !tl && !tr && bl && !br {
            let a1 = getAngle([r1[0], r1[3]], [r2[2], r2[1]])
            let a2 = getAngle([r1[2], r1[1]], [r2[2], r2[1]])
            if a1 > a2 { return Direction.up } else { return Direction.right }
        }
        if !tl && !tr && !bl && br {
            let a1 = getAngle([r1[2], r1[3]], [r2[0], r2[1]])
            let a2 = getAngle([r1[0], r1[1]], [r2[0], r2[1]])
